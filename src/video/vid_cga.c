...
cga_update_latch(cga_t *cga)
{
    if (cga->precise_enabled) {
        mc6845_core_latch_lightpen(&cga->precise_crtc);
    } else {
...
cga_in(uint16_t addr, void *priv)
{
    ...
    switch (addr) {
        case CGA_REGISTER_CRTC_INDEX:
            ...
            break;
        case CGA_REGISTER_CRTC_DATA:
            ret = cga->precise_enabled
                ? mc6845_core_select_read(&cga->precise_crtc, cga->crtcreg)
                : cga->crtc[cga->crtcreg];
            break;
...
cga_write(uint32_t addr, uint8_t val, void *priv)
{
    ...
    if (cga->snow_enabled) {
        int offset                  = cga->precise_enabled ? ((cga->precise_crtc.hcc * 2) & 0xfc) : (((timer_get_remaining_u64(&cga->timer) / CGACONST) * 2) & 0xfc);
...
cga_poll(void *priv)
{
    ...
    if (cga->precise_enabled)
        cga_poll_precise(priv);
    else
        cga_poll_legacy(priv);
}
